...
/**
 * Downstrum: all 6 strings, low to high, full volume
 * Sounds like a proper downward guitar strum
 */
export function playDownstrum(chordId, volume = 0.18) {
  const ctx = getCtx();
  const voicing = CHORD_VOICINGS[chordId];
  if (!voicing) return;
  const now = ctx.currentTime;
  // Low to high (string 6 → 1), 20ms apart = natural strum feel
  voicing.forEach((fret, i) => {
    if (fret === null) return;
    const freq = BASE_FREQS[i] * semi(fret);
    playString(ctx, freq, now + i * 0.02, 1.6, volume);
  });
}

/**
 * Upstrum: only high strings (1–3), high to low, lighter volume
 * Sounds like a quick upward catch of the treble strings
 */
export function playUpstrum(chordId, volume = 0.10) {
  const ctx = getCtx();
  const voicing = CHORD_VOICINGS[chordId];
  if (!voicing) return;
  const now = ctx.currentTime;
  // Only top 3 strings (indices 3,4,5 = G,B,E), played high to low
  [5, 4, 3].forEach((i, order) => {
    const fret = voicing[i];
    if (fret === null) return;
    const freq = BASE_FREQS[i] * semi(fret);
    playString(ctx, freq, now + order * 0.015, 1.0, volume);
  });
}
